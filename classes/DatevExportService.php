 ...
 *
 * One booking per paid/issued order: the money account (Soll) to the
 * voucher-liability account (Haben), with NO VAT — the multi-purpose voucher sale
 * is not a taxable supply (§ 3 Abs. 15 UStG). The redemption is booked by the
 * restaurant's TSE cash register, not here. The whole paid total (incl. any
 * shipping fee) is booked to the liability account; splitting the fee is left to
 * the operator's tax advisor (see docs/umsatzsteuerliche-behandlung.md).
 *
...
class DatevExportService
{
    ...
    public static function export(iterable $orders, Carbon $from, Carbon $to, ?Carbon $now = null): string
    {
        ...
        foreach ($orders as $order) {
            ...
            $row[0]  = self::amount($order->total_cents);          // Umsatz (always positive)
            ...
            // BU-Schlüssel (index 8) stays empty: the multi-purpose sale has no VAT.
